...
export default function (streamId) {
    const playPosition = `PT${randomIntBetween(1, 100)}S`
    const sendStreamPositionResponse = http.post(`${STREAMING_SERVICE_URL}/actions/send?stream_id=${streamId}play_position=${playPosition}`)
    check(sendStreamPositionResponse, {
